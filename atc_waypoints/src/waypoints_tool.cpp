...
namespace atc_waypoints
{
  ...
  void WaypointTool::updateTopic()
  {
    try
    {
      pub_ = nh_.advertise<atc_msgs::waypoint_msg>("/waypoint", 1);
    }
  ...
  void WaypointTool::onPoseSet(double x, double y, double theta)
  {
    ...
    else
    {
      ...
      ROS_INFO("\nSetting waypoint %s:\n  Position(%.3f, %.3f, %.3f), Orientation(%.3f, %.3f, %.3f, %.3f) , Angle:%.3f deg\n",
			   ...
			   atc_utils::to_degrees(theta));
      atc_msgs::waypoint_msg msg;
      msg.name = name_wp;
...
PLUGINLIB_EXPORT_CLASS(atc_waypoints::WaypointTool, rviz::Tool)
